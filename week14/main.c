...
int getWinner(void)
{
	...
	for (int i=0; i<N_PLAYER; i++)
	{
		if(player_status[i]==PLAYERSTATUS_LIVE && player_coin[i] > maxcoins)
		{
...
// ----- EX. 4 : player ------------
        ...
        {
        	printf("%s found a coin!\n", player_name[turn]);
        	player_coin[turn]++;
			board_setBoardStatus(player_position[turn], BOARDSTATUS_NOK);
	    }
 // ----- EX. 4 : player ------------       
...
// ----- EX. 6 : game end ------------
    ...
    if(winner == -1)
    {
    	printf("All players have died! No one wins.\n");
	}
	else
	{
		printf("%i players are alive! The winner is %s with %i coins!\n", alivePlayers, player_name[winner], player_coin[winner]);
	}
